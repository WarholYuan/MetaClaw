...
from common.brand import DEFAULT_AGENT_WORKSPACE, DEFAULT_ENV_FILE
from common.log import logger
from common.utils import expand_path
from models.openai_compatible_bot import OpenAICompatibleBot


class ToolCallingBotAdapter(OpenAICompatibleBot):
    """
    Adapter that wraps a bot to add tool calling support.
    
    Delegates all attribute access to the wrapped bot except:
    - call_with_tools (provided by OpenAICompatibleBot)
    - get_api_config (provided by this adapter)
    
    This replaces the old approach of bot_instance.__class__ = EnhancedBot,
    which was fragile and broke type checks.
    """
    ...
    def get_api_config(self):
        ...
        return {
            'api_key': conf().open_ai_api_key,
            'api_base': conf().open_ai_api_base,
            'model': conf().model or "gpt-3.5-turbo",
            'default_temperature': conf().temperature or 0.9,
            'default_top_p': conf().top_p or 1.0,
            'default_frequency_penalty': conf().frequency_penalty or 0.0,
            'default_presence_penalty': conf().presence_penalty or 0.0,
        }
    
    def __getattr__(self, name):
        """Delegate all unknown attributes to wrapped bot."""
        if name.startswith('_'):
            raise AttributeError(name)
        return getattr(self._wrapped_bot, name)
    
    def __repr__(self):
        return f"<ToolCallingBotAdapter wrapping {type(self._wrapped_bot).__name__}>"


def add_openai_compatible_support(bot_instance):
    """
    Add OpenAI-compatible tool calling support via Adapter pattern.

    Returns a ToolCallingBotAdapter wrapping the bot for bot types
    that don't natively support tools. Bots with native support (DeepSeek,
    Claude, Gemini, Doubao, etc.) pass through unchanged.

    Uses models.bot_factory.supports_tools() for the authoritative list.
    """
    if hasattr(bot_instance, 'call_with_tools'):
        logger.debug(
            f"[AgentBridge] {type(bot_instance).__name__} "
            f"already has native tool calling support"
        )
        return bot_instance

    # Use shared supports_tools() from bot_factory for consistent classification
    from models.bot_factory import supports_tools
    from bridge.bot_manager import resolve_bot_type
    from config import conf
    bot_type = resolve_bot_type(conf().get("model", ""))
    if supports_tools(bot_type):
        logger.debug(
            f"[AgentBridge] {type(bot_instance).__name__} (bot_type={bot_type}) "
            f"has native tool support — adapter skipped"
        )
        return bot_instance

    adapter = ToolCallingBotAdapter(bot_instance)
    logger.info(
        f"[AgentBridge] Enhanced {type(bot_instance).__name__} "
        f"with tool calling via Adapter"
    )
    return adapter
...
class AgentLLMModel(LLMModel):
    ...
    def _resolve_bot_type(self, model_name: str) -> str:
        """Resolve bot type — delegates to shared bot_manager.resolve_bot_type."""
        from bridge.bot_manager import resolve_bot_type
...
class AgentBridge:
    ...
    def _refresh_conditional_tools(agent):
        ...
        For example, web_search should only be present when BOCHA_API_KEY is set.
        """
        try:
        ...
        try:
            from agent.tools.opencli.opencli_tool import OpenCLITool

            has_tool = any(t.name == "opencli" for t in agent.tools)
            available = OpenCLITool.is_available()

            if available and not has_tool:
                tool = OpenCLITool()
                tool.model = agent.model
                agent.tools.append(tool)
                logger.info("[AgentBridge] opencli tool added (binary now available)")
            elif not available and has_tool:
                agent.tools = [t for t in agent.tools if t.name != "opencli"]
                logger.info("[AgentBridge] opencli tool removed (binary no longer available)")
        except Exception as e:
            logger.debug(f"[AgentBridge] Failed to refresh opencli tool: {e}")
