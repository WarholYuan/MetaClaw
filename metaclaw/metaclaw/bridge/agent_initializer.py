...
class AgentInitializer:
    ...
    def _setup_memory_system(self, workspace_root: str, session_id: Optional[str] = None):
        ...
        try:
            ...
            # Initialize embedding provider (prefer OpenAI)
            ...
            if embedding_provider is None:
                logger.info("[AgentInitializer] Embedding not available, memory will use keyword search only")
            ...
            try:
                ...
                if tool_name == "web_search":
                    ...
                    if not WebSearch.is_available():
                        logger.debug("[AgentInitializer] WebSearch skipped - no BOCHA_API_KEY")
                        continue

                # Skip opencli if the binary is not on PATH
                if tool_name == "opencli":
                    from agent.tools.opencli.opencli_tool import OpenCLITool
                    if not OpenCLITool.is_available():
                        logger.debug("[AgentInitializer] OpenCLI skipped - opencli binary not found (run: npm install -g @jackwener/opencli)")
                        continue
