---
title: 機能詳細
description: MetaClaw の長期記憶、タスク計画、Skill システム、CLI コマンド、ブラウザツールの詳細
---
...
初回起動時、Agent はユーザーに重要な情報を自発的に尋ね、ワークスペース（デフォルト `~/metaclaw`）に記録します。これには Agent の設定、ユーザーの身元情報、記憶ファイルが含まれます。
...
Agent は会話中に価値ある情報を自動的にナレッジページとして整理し、相互参照とインデックスを維持します。Web コンソールではドキュメントの閲覧とナレッジグラフの可視化が可能です。ナレッジはワークスペースの `~/metaclaw/knowledge/` ディレクトリに保存されます。
...
組み込みの `browser` ツールにより、Agent は Chromium ブラウザを制御して Web ページへのアクセス、フォームの入力、要素のクリック、スクリーンショットの撮影が可能です。動的 JS レンダリングページにも対応しています。`metaclaw install-browser` でワンコマンドインストール、サーバー（ヘッドレス）とデスクトップ環境に自動対応します：
...
- **[Skill Hub](https://skills.metaclaw.ai/)：** オープンな Skill マーケットプレイス。公式推奨、コミュニティ、サードパーティの Skill を収録。ワンコマンドでインストール可能。
- **組み込み Skill：** ソースパッケージの `metaclaw/metaclaw/skills/` にあり、製品と一緒に配布されます。組み込み Skill は依存条件（API キー、システムコマンドなど）に基づいて自動的に有効化されます。
- **カスタム Skill：** ユーザーが会話を通じて作成し、Agent ワークスペース（デフォルト `~/.metaclaw/workspace/skills/`）に保存されます。あらゆる複雑なビジネスプロセスやサードパーティ連携を実装できます。同名の場合はカスタム Skill が組み込み Skill より優先されます。

Skill のインストール：`/skill install <名前>` または `metaclaw skill install <名前>`。Skill Hub、GitHub、ClawHub、URL などからインストール可能。
...
[skills.metaclaw.ai](https://skills.metaclaw.ai/) で利用可能なすべての Skill を閲覧するか、会話内でコマンドを実行できます：
...
MetaClaw はサービス管理、Skill インストール、設定変更などをカバーする2つのコマンドインターフェースを提供します：

- **ターミナル CLI：** システムターミナルで `metaclaw <コマンド>` を実行。`start`、`stop`、`restart`、`update`、`status`、`logs`、`skill` などをサポート。
- **チャットコマンド：** 会話内で `/<コマンド>` を入力。Web コンソールでは `/` を入力するとコマンドメニューが表示されます。

```bash
metaclaw start              # サービスを開始
metaclaw stop               # サービスを停止
metaclaw update             # 更新して再起動
metaclaw skill install pptx # Skill をインストール
metaclaw install-browser    # ブラウザツールをインストール
```

詳細は [コマンド一覧](https://docs.metaclaw.ai/ja/cli) を参照してください。
