---
title: 手动安装
description: 手动部署 MetaClaw（源码 / Docker）
---
...
```bash
git clone https://github.com/WarholYuan/MetaClaw.git
cd MetaClaw/
```

<Tip>
  若遇到网络问题可使用国内仓库地址：https://gitee.com/WarholYuan/MetaClaw

> 注：此为上游仓库地址，如果你使用本分支的代码，请改为对应的仓库地址。
</Tip>
...
<Note>
  此步骤为推荐操作。安装后可以使用 `metaclaw start`、`metaclaw stop`、`metaclaw update` 等命令管理服务，也可以使用 `metaclaw skill` 管理技能。如果不安装 CLI，可以使用 `./run.sh` 或 `python3 app.py` 运行。
</Note>
...
| `agent_workspace` | - | Agent 工作空间路径 | `~/metaclaw` |
...
<Tip>
  全部配置项可在项目 [`config/__init__.py`](https://github.com/WarholYuan/MetaClaw/blob/master/config/__init__.py) 文件中查看。Docker 部署时，配置项名称需转为大写环境变量格式。
</Tip>
