---
title: Features
description: MetaClaw long-term memory, task planning, skills system, CLI commands, and browser tool in detail
---
...
On first launch, the Agent proactively asks the user for key information and records it in the workspace (default `~/metaclaw`) — including agent settings, user identity, and memory files.
...
The Agent automatically organizes valuable information from conversations into knowledge pages, maintaining cross-references and indexes. The Web console provides document browsing and knowledge graph visualization. Knowledge is stored in `~/metaclaw/knowledge/` within the workspace.
...
The built-in `browser` tool allows the Agent to control a Chromium browser to visit web pages, fill forms, click elements, and take screenshots, with support for dynamic JS-rendered pages. Run `metaclaw install-browser` to install with one command, automatically adapting to server (headless) and desktop environments:
...
- **[Skill Hub](https://skills.metaclaw.ai/):** An open skill marketplace featuring official, community, and third-party skills. Install with one command.
- **Built-in skills:** Located in the source package at `metaclaw/metaclaw/skills/` and shipped with the product. Built-in skills are automatically enabled based on dependency conditions (API keys, system commands, etc.).
- **Custom skills:** Created by users through conversation and stored in the Agent workspace (default `~/.metaclaw/workspace/skills/`), capable of implementing any complex business process or third-party integration. A custom skill with the same name overrides the built-in one.

Install skills: `/skill install <name>` or `metaclaw skill install <name>`, supporting Skill Hub, GitHub, ClawHub, URL, and more.
...
Visit [skills.metaclaw.ai](https://skills.metaclaw.ai/) to browse all available skills, or use commands in conversation:
...
MetaClaw provides two command interaction methods, covering service management, skill installation, configuration, and more:

- **Terminal CLI:** Run `metaclaw <command>` in the system terminal, supporting `start`, `stop`, `restart`, `update`, `status`, `logs`, `skill`, etc.
- **Chat commands:** Type `/<command>` in conversation. The Web console shows a command menu when you type `/`.

```bash
metaclaw start              # Start service
metaclaw stop               # Stop service
metaclaw update             # Update and restart
metaclaw skill install pptx # Install a skill
metaclaw install-browser    # Install browser tool
```

See [Command Overview](https://docs.metaclaw.ai/en/cli) for details.
