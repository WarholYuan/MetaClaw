...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning MetaClaw project...${NC}"

    if [ -d "MetaClaw" ]; then
        echo -e "${YELLOW}⚠️  Directory 'MetaClaw' already exists.${NC}"
        read -p "Choose action: overwrite(o), backup(b), or quit(q)? [press Enter for default: b]: " choice
        choice=${choice:-b}
        case "$choice" in
            o|O)
                echo -e "${YELLOW}🗑️  Overwriting 'MetaClaw' directory...${NC}"
                rm -rf MetaClaw
                ;;
            b|B)
                backup_dir="MetaClaw_backup_$(date +%s)"
                echo -e "${YELLOW}🔀 Backing up to '$backup_dir'...${NC}"
                mv MetaClaw "$backup_dir"
                ;;
            q|Q)
    ...
    cd MetaClaw || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
install_dependencies() {
    ...
    # Register `metaclaw` CLI command via editable install
    echo -e "${YELLOW}Registering metaclaw CLI...${NC}"
    set +e
    $PYTHON_CMD -m pip install -e . $PIP_EXTRA_ARGS $PIP_MIRROR > /dev/null 2>&1
    if command -v metaclaw &> /dev/null; then
        echo -e "${GREEN}✅ metaclaw CLI registered.${NC}"
    else
        echo -e "${YELLOW}⚠️  metaclaw CLI not in PATH, you can still use: $PYTHON_CMD -m cli.cli${NC}"
    fi
    set -e
}
...
start_project() {
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} MetaClaw is now running in background!${NC}"
...
cmd_start() {
    ...
    if has_metaclaw_cli; then
        ...
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} MetaClaw is already running (PID: $(get_pid))${NC}"
...
cmd_stop() {
    if has_metaclaw_cli; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} Stopping MetaClaw...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} MetaClaw is not running${NC}"
            return
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} MetaClaw stopped${NC}"
    fi
}
...
# Reconfigure
cmd_config() {
    echo -e "${YELLOW}${EMOJI_WRENCH} Reconfiguring MetaClaw...${NC}"
...
install_mode() {
    ...
    echo ""
    read -p "Start MetaClaw now? [Y/n]: " start_now
