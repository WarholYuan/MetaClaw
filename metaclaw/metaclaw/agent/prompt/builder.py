...
def _build_workspace_section(workspace_dir: str, language: str) -> List[str]:
    ...
    lines = [
        ...
        f"   - ✅ 正确: 例如 `~/projects/myapp`、`/usr/local/`",
        f"   - ❌ 错误: 假设相对路径会指向其他目录",
        "",
        "3. **路径解析示例**:",
        f"   - 相对路径 `memory/` → 实际路径 `{workspace_dir}/memory/`",
        f"   - 默认生成文件 `report.md` → 实际路径 `{workspace_dir}/tmp/report.md`",
        f"   - 绝对路径 `~/projects/myapp/docs/` → 实际路径 `~/projects/myapp/docs/`",
        "",
        "4. **生成文件默认位置**: 运行过程中创建的报告、草稿、下载附件、截图、临时网页和转换产物默认写入 `tmp/`。只有用户明确要求长期保留，或属于记忆/知识/技能/规则文件时，才写入对应目录",
        "",
        "5. **不确定时**: 先用 `bash pwd` 确认当前目录，或用 `ls .` 查看当前位置",
        "",
        "**重要说明 - 文件已自动加载**:",
        "",
