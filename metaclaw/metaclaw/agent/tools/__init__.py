"""
Agent Tools — LLM-callable functions exposed via OpenAI function calling.

These tools are invoked by the LLM during conversation to perform actions
(bash commands, file I/O, web search, memory, etc.).

Distinction from plugins/:
  - agent/tools/ → Called by LLM via function calling (Agent decides when)
  - plugins/     → Message lifecycle hooks (triggered before/after Agent)

To add a new tool: create a class extending BaseTool, implement execute(),
and import it below.
"""
...
# OpenCLITool (requires opencli npm package on PATH)
def _import_opencli_tool():
    from common.log import logger
    try:
        from agent.tools.opencli.opencli_tool import OpenCLITool
        return OpenCLITool
    except Exception as e:
        logger.error(f"[Tools] OpenCLITool failed to load: {e}")
        return None

OpenCLITool = _import_opencli_tool()
