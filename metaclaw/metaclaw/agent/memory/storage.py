...
class MemoryStorage:
    ...
    def _init_db(self):
        ...
        try:
            ...
            try:
                ...
                if result[0] != 'ok':
                    logger.warning(f"⚠️  Database integrity check failed: {result[0]}")
                    logger.warning(f"   Recreating database...")
                    self.conn.close()
                    self.conn = None
                ...
                logger.warning(f"⚠️  Database is corrupted, recreating...")
                if self.conn:
                    self.conn.close()
                    self.conn = None
                self.db_path.unlink(missing_ok=True)
            ...
            logger.warning(f"⚠️  Unexpected error during database initialization: {e}")
            raise
    ...
    def search_vector(
        ...
        if user_id:
            # nosec B608: f-string only inserts parameterized placeholders ('?'), not values
            query = f"""
                SELECT * FROM chunks 
    ...
    def _search_like(
        ...
        if user_id:
            sql_query = f"""  # nosec B608: f-string only inserts parameterized placeholders, not values
                SELECT * FROM chunks
                WHERE ({where_clause})
