...
class WebChannel(ChatChannel):
    ...
    def startup(self):
        host = conf().get("web_host", "127.0.0.1")
        ...
        if str(host) in ("127.0.0.1", "localhost", "::1"):
            logger.info("[WebChannel] 🔒 当前仅本机可访问，如需远程访问请设置 web_host 为 0.0.0.0 并配置 web_password")
        else:
...
class ConfigHandler:
    ...
    def GET(self):
        ...
        try:
            ...
            title = APP_NAME if use_agent else "AI Assistant"
    ...
    def POST(self):
        ...
        try:
            ...
            # Reset Bridge so that cached bot instance reflects the new config
            # after the user switches bot_type / model in UI.
    ...
    def _poll_status(self):
        ...
        if qr_status == "confirmed":
            ...
            cred_path = os.path.expanduser(
                conf().get("weixin_credentials_path", DEFAULT_WEIXIN_CREDENTIALS_PATH)
            )
...
def _get_workspace_root():
    ...
    from common.utils import expand_path
    return expand_path(conf().get("agent_workspace", DEFAULT_AGENT_WORKSPACE))
