        ...
        try:
            ...
            if img_url.startswith("file://"):
                ...
                with open(local_path, "rb") as file:
                    upload_response = requests.post(upload_url, files={"image": file}, data=data, headers=headers, timeout=(5, 30))
            ...
            logger.error(f"[FeiShu] upload image timeout: {img_url}")
            return None
        except requests.exceptions.ConnectionError:
            logger.error(f"[FeiShu] upload image connection error: {img_url}")
            return None
        except Exception as e:
            logger.error(f"[FeiShu] upload image exception: {e}")
            return None
