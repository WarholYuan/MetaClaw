...
function Install-Project {
    ...
    try {
        ...
        git clone --depth 10 --progress "https://github.com/WarholYuan/MetaClaw.git" 2>&1 | ForEach-Object { Write-Host $_ }
    ...
    if (-not $cloneOk) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://gitee.com/WarholYuan/MetaClaw.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "MetaClaw") { Remove-Item -Recurse -Force "MetaClaw" }
    ...
    if (-not $cloneOk) {
        ...
        Write-Err "You can also manually clone: git clone https://gitee.com/WarholYuan/MetaClaw.git"
        Read-Host "Press Enter to exit"
        exit 1
    }
...
function Update-Project {
    ...
    if (Test-Path "$BaseDir\.git") {
        ...
        if ($pullExit -ne 0) {
            ...
            git remote set-url origin https://gitee.com/WarholYuan/MetaClaw.git 2>&1 | Out-Null
            git pull 2>&1 | Out-Null
            $ErrorActionPreference = $prevEAP
        }
    } else {
