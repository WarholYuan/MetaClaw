"""
Command plugin - Intercept CLI/slash commands in chat messages.
...
# Known top-level subcommands that CLI supports
KNOWN_COMMANDS = {
...
@plugins.register(
    name="metaclaw_cli",
    desc="Handle CLI/slash commands in chat messages",
    version="0.1.0",
    author="MetaClaw",
    desire_priority=1000,
)
class MetaClawCliPlugin(Plugin):

    def __init__(self):
        super().__init__()
        self.handlers[Event.ON_HANDLE_CONTEXT] = self.on_handle_context
        logger.debug("[MetaClawCli] initialized")
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        cmd, args = parsed
        logger.info(f"[MetaClawCli] intercepted command: {cmd} {args}")
    ...
    def _parse_command(self, content: str):
        """
        Parse CLI command from message text.

        Supported formats:
          <cli> <command> [args...]   e.g. "metaclaw skill list"
          /<command> [args...]      e.g. "/skill list"

        Returns (command, args_string) or None if not a CLI command.
        """
        parts = None
        ...
        elif content.startswith(f"{CLI_NAME} "):
            rest = content[len(CLI_NAME) + 1:].strip()
            if rest:
                parts = rest.split(None, 1)
        if not parts:
            return None
    ...
    def execute(self, query: str, session_id: str = "") -> str:
        """Execute a CLI/slash command string without a channel context.
    ...
    def _dispatch(self, cmd: str, args: str, e_context: EventContext, session_id: str = "") -> str:
        if cmd in CLI_ONLY_COMMANDS:
            return f"⚠️ `{CLI_NAME} {cmd}` 只能在命令行终端中执行。\n请在终端运行: {CLI_NAME} {cmd}"
        ...
        if handler:
            try:
                ...
                logger.error(f"[MetaClawCli] command '{cmd}' failed: {e}")
                return f"命令执行失败: {e}"
    ...
    def _cmd_help(self, args: str, e_context, **_) -> str:
        lines = [
            ...
            "",
            f"💡 也可以用 {CLI_NAME} <command> 代替 /<command>",
        ]
        return "\n".join(lines)
    ...
    def _config_set(self, key: str, value_str: str) -> str:
        ...
        logger.info(f"[MetaClawCli] config update: {updates}, synced envs: {synced_envs}")

        try:
            load_config()
        except Exception as e:
            logger.warning(f"[MetaClawCli] config reload warning: {e}")
    ...
    # ------------------------------------------------------------------
    # install-browser (shared logic with CLI install-browser)
    # ------------------------------------------------------------------
    ...
    def _send_install_progress(e_context, text: str) -> None:
        ...
        if e_context is None:
            logger.info(f"[MetaClawCli] install-browser: {text}")
            return
        try:
            ...
            logger.warning(f"[MetaClawCli] install-browser progress send failed: {e}")
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        ...
        if args.strip():
            return (
                "用法: /install-browser\n\n"
                f"无需参数，等同于终端执行 `{CLI_NAME} install-browser`。\n"
        ...
        if code != 0:
            return (
                ...
                f"也可在终端执行 `{CLI_NAME} install-browser`。"
            )
        return f"✅ 安装流程已结束。请重启 {APP_NAME} 后使用 browser 工具（进度见上方消息）。"
    ...
    def _refresh_skill_manager(self):
        ...
        try:
            ...
            logger.debug(f"[MetaClawCli] skill refresh skipped: {e}")
    ...
    def _memory_dream(self, days: int, e_context, session_id: str) -> str:
        ...
        def _run():
            try:
                ...
                logger.warning(f"[MetaClawCli] /memory dream failed: {e}")
    ...
    def _memory_dream_sync(self, flush_mgr, days: int) -> str:
        ...
        try:
            ...
            logger.warning(f"[MetaClawCli] /memory dream sync failed: {e}")
            return f"❌ 记忆蒸馏失败: {e}"
    ...
    def _notify(e_context, text: str):
        ...
        if e_context is None:
            logger.info(f"[MetaClawCli] {text}")
            return
        try:
            channel = e_context["channel"]
            context = e_context["context"]
            ...
            logger.warning(f"[MetaClawCli] notify failed: {e}")
    ...
    def get_help_text(self, **kwargs):
        return f"在对话中使用 /help 或 {CLI_NAME} help 查看可用命令"
