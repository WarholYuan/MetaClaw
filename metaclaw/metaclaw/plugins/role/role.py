...
class Role(Plugin):
    def __init__(self):
        ...
        try:
            ...
            if isinstance(e, FileNotFoundError):
                logger.warn(f"[Role] init failed, {config_path} not found, ignore or check plugin config.")
            else:
                logger.warn("[Role] init failed, ignore or check plugin config.")
            raise e
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if btype not in [const.OPEN_AI, const.OPENAI, const.CHATGPT, const.CHATGPTONAZURE, const.QWEN_DASHSCOPE, const.XUNFEI, const.BAIDU, const.ZHIPU_AI, const.MOONSHOT, const.MiniMax, const.MODELSCOPE]:
