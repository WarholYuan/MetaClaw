...
class Godcmd(Plugin):
    def __init__(self):
        ...
        if gconf["password"] == "":
            ...
            logger.warning("[Godcmd] 未设置口令，已生成临时口令（可通过 #auth 指令输入进行认证）")
        else:
        ...
        if content.startswith("#"):
            ...
            if any(cmd in info["alias"] for info in COMMANDS.values()):
                ...
                if cmd == "auth":
                    ...
                    if bottype in [const.OPEN_AI, const.OPENAI, const.CHATGPT, const.CHATGPTONAZURE, const.BAIDU, const.XUNFEI, const.QWEN, const.QWEN_DASHSCOPE, const.GEMINI, const.ZHIPU_AI, const.CLAUDEAPI]:
                ...
                if isadmin:
                    if isgroup:
                        ...
                        if cmd == "stop":
                            ...
                            if bottype in [const.OPEN_AI, const.OPENAI, const.CHATGPT, const.CHATGPTONAZURE,
