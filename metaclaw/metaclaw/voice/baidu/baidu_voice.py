...
class BaiduVoice(Voice):
    ...
    def _long_text_synthesis(self, text):
        ...
        create_resp = requests.post(create_url, headers=headers, json=payload, timeout=30).json()
        ...
        for _ in range(100):
            time.sleep(3)
            resp = requests.post(query_url, headers=headers, json={"task_ids":[task_id]}, timeout=30)
            result = resp.json()
