...
class OpenAICompatibleBot:
        ...
        try:
            ...
            if stream:
                def error_generator():
                    yield {
                        "error": True,
                        "message": "Network timeout or connection failure. Please retry.",
                        "status_code": 503
                    }
            ...
            logger.error(f"[{self.__class__.__name__}] call_with_tools API error: {e}")
            if stream:
                def error_generator():
                    yield {
                        "error": True,
                        "message": str(e),
                        "status_code": getattr(e, 'http_status', 502)
                    }
                return error_generator()
            else:
                return {
            ...
            logger.error(f"[{self.__class__.__name__}] call_with_tools rate limit: {e}")
            if stream:
                def error_generator():
                    yield {
                        "error": True,
    ...
    def _handle_sync_response(self, request_params, api_key, api_base):
        ...
        try:
            ...
            logger.error(f"[{self.__class__.__name__}] sync API error: {e}")
            return {
                "error": True,
                "message": str(e),
                "status_code": getattr(e, 'http_status', 502)
            ...
            logger.error(f"[{self.__class__.__name__}] sync rate limit: {e}")
            return {
                "error": True,
                "message": "Rate limit exceeded. Please wait and retry.",
                "status_code": 429
    ...
    def _handle_stream_response(self, request_params, api_key, api_base):
        ...
        try:
            ...
            logger.error(f"[{self.__class__.__name__}] stream API error: {e}")
            yield {
                "error": True,
                "message": str(e),
                "status_code": getattr(e, 'http_status', 502)
            ...
            logger.error(f"[{self.__class__.__name__}] stream rate limit: {e}")
            yield {
                "error": True,
                "message": "Rate limit exceeded. Please wait and retry.",
                "status_code": 429
    ...
    def call_vision(self, image_url: str, question: str,
        ...
        try:
            ...
            logger.error(f"[{self.__class__.__name__}] call_vision network error: {e}")
            return {"error": True, "message": "Network timeout or connection failure. Please retry.", "status_code": 503}
        except openai.error.APIError as e:
            logger.error(f"[{self.__class__.__name__}] call_vision API error: {e}")
            return {"error": True, "message": str(e), "status_code": getattr(e, 'http_status', 502)}
        except openai.error.RateLimitError as e:
            logger.error(f"[{self.__class__.__name__}] call_vision rate limit: {e}")
            return {"error": True, "message": "Rate limit exceeded. Please wait and retry.", "status_code": 429}
        except Exception as e:
