...
class BaiduUnitBot(Bot):
    def reply(self, query, context=None):
        ...
        logger.debug(f"[BaiduUnitBot] post_data: {post_data}")
        headers = {"content-type": "application/x-www-form-urlencoded"}
        response = requests.post(url, data=post_data.encode(), headers=headers, timeout=30)
        if response:
            reply = Reply(
                ReplyType.TEXT,
    ...
    def get_token(self):
        ...
        response = requests.get(host, timeout=30)
        if response:
            logger.debug(f"[BaiduUnitBot] token response: {response.json()}")
