...
import click
import requests
from common.brand import CLI_NAME
...
def _clone_repo(git_url: str):
    ...
    """
    tmp_dir = tempfile.mkdtemp(prefix=f"{CLI_NAME}-skill-")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with tarfile.open(tar_path, "r:gz") as tf:
            ...
            for member in tf.getmembers():
                ...
                if not resolved.startswith(extract_real):
...
# ------------------------------------------------------------------
# skill install (CLI thin wrapper)
# ------------------------------------------------------------------
@skill.command()
@click.argument("name")
def install(name):
    ...
    Examples:
      ...
      metaclaw skill install larksuite/cli#skills/lark-im  (single skill by subpath)

      metaclaw skill install https://github.com/owner/repo

      metaclaw skill install https://gitlab.com/org/repo

      metaclaw skill install git@github.com:owner/repo.git

      metaclaw skill install ./my-local-skills             (local directory)
